...
const ProductDetails = () => {
	...
	if (loading) {
		<section className="mt-[82px] mx-[50px]">
			<div className="grid grid-cols-2 mb-[180px] gap-10">
				<div className="">
					<img
						src={fallbackProductDetails?.image}
						className="aspect-[0.9] h-[618px] w-[512px] object-cover"
						alt=""
					/>
				</div>
				<div className="">
					<Text variant="heading-one">{fallbackProductDetails?.name}</Text>
					<Text variant="subheading-two" className="my-7">
						$ {fallbackProductDetails?.price}
					</Text>
					<Text variant="body-two">{fallbackProductDetails?.description}</Text>
					<Button
						size="small"
						className="mt-14"
						onClick={() => {
							if (!fallbackProductDetails) return;
							const cartItem: RawCartItem = {
								...
								product: fallbackProductDetails?._id,
							};
							addItemToCart({
								...cartItem,
							});
